...
function isPro(req, res, next) {
  ...
  if (req.user.plan === 'pro' || req.user.plan === 'expert') {
    return next();
  }
  res
    .status(403)
    .json({ error: 'Pro plan required. Upgrade to access this feature.', upgrade: true });
}
...
function isExpert(req, res, next) {
  ...
  if (req.user.plan === 'expert') {
    return next();
  }
  res
    .status(403)
    .json({ error: 'Expert plan required. Upgrade to access this feature.', upgrade: true });
}
