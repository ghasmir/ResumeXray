...
function parseInstitutionDateLine(line) {
  ...
  if (!dateMatch || !EDUCATION_INSTITUTION_RE.test(clean)) return null;
...
function looksLikeWrappedBulletContinuation(line) {
  ...
  if (/^[A-Z]/.test(clean) && /[.!?]$/.test(clean) && clean.split(/\s+/).length >= 8) return false;
...
function isLikelyNarrativeBulletLine(line) {
  const clean = String(line || '').trim();
  if (!clean || isDateOnlyLine(clean) || looksLikeStandaloneLocationLine(clean)) return false;
...
function shouldStartImplicitBullet(line, previousBullet = '') {
  ...
  if (looksLikeWrappedBulletContinuation(clean)) return false;
...
function structureExperience(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (BULLET_RE.test(line)) {
      ...
      } else if (current && current.bullets.length === 0 && looksLikeStandaloneLocationLine(line) && !current.location) {
        current.location = line.trim();
