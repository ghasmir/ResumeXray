...
Handlebars.registerHelper('formatContact', function (contactParts) {
  if (!contactParts || !Array.isArray(contactParts)) {
    return '';
  }
...
Handlebars.registerHelper('boldMetrics', function (text) {
  ...
  let formatted = escapeHtml(text); // Escape HTML first
  formatted = formatted.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
  // Bold numbers with units: "45%", "$2.3M", "150+", "3x"
  formatted = formatted.replace(
    /(\$?\d[\d,.]*[%+xX]?(?:\s*(?:million|billion|users|customers|clients|requests|orders|employees|team\s+members))?)/g,
    '<strong>$1</strong>'
  );
 ...
 * @param {object} options - { density, jobUrl, template }
 * @returns {string} Complete HTML document
 */
function renderTemplate(templateName, data, options = {}) {
  const { density = 'standard', jobUrl = '', template: selectedTemplate = 'refined' } = options;
  ...
  const context = {
    ...
    strengths: data.sections?.strengths || [],
    projects:
      rawProjects[0] && typeof rawProjects[0] === 'object'
        ? rawProjects
        : structureProjects(rawProjects),
...
function buildContactArray(contactString) {
  ...
  if (Array.isArray(contactString)) {
 ...
 */
const DATE_RANGE_RE =
  /\b\d{2}\/\d{4}\b.*[-–—]|[-–—].*\b\d{2}\/\d{4}\b|\b(19|20)\d{2}\b.*[-–—].*\b(Present|(19|20)\d{2})\b/i;
...
function looksLikeStandaloneLocationLine(line) {
  ...
  return /[A-Za-z.' -]+,\s*[A-Za-z.' -]+(?:\s*\((?:Remote|Hybrid|On-?site)\))?$/i.test(clean);
}
...
function parseInstitutionDateLine(line) {
  const clean = String(line || '').trim();
  if (!clean) {
    return null;
  }

  const pipeMatch = clean.match(/^(.+?)\s*\|\s*((?:\d{2}\/)?(?:19|20)\d{2}|Present)$/i);
  if (pipeMatch) {
    return {
      school: pipeMatch[1].trim(),
      dates: pipeMatch[2].trim(),
    };
  }

  const dateMatch = clean.match(/((?:\d{2}\/)?(?:19|20)\d{2}|Present)\s*$/i);
  if (!dateMatch || !EDUCATION_INSTITUTION_RE.test(clean)) {
    return null;
  }
...
function isLikelyEntryHeader(line, nextLine = '') {
  ...
  if (BULLET_RE.test(clean) || clean.length > 140 || /[.!?]$/.test(clean)) {
    return false;
  }

  if (nextLine && isDateOnlyLine(nextLine)) {
    return (
      ENTRY_SEPARATOR_RE.test(clean) || isLikelyRoleTitle(clean) || looksLikeCompanyText(clean)
    );
  }

  if (DATE_RANGE_RE.test(clean) || isDateOnlyLine(clean)) {
    return true;
  }
  ...
  if (ENTRY_SEPARATOR_RE.test(clean)) {
    ...
    return (
      ...
      isLikelyRoleTitle(parsed.title) &&
      looksLikeCompanyText(parsed.company)
    );
  }
...
function looksLikeWrappedBulletContinuation(line) {
  ...
  if (/^[A-Z]/.test(clean) && /[.!?]$/.test(clean) && clean.split(/\s+/).length >= 8) {
    return false;
  }
  if (/^[a-z]/.test(clean)) {
    return true;
...
function isLikelyNarrativeBulletLine(line) {
  const clean = String(line || '').trim();
  if (!clean || isDateOnlyLine(clean) || looksLikeStandaloneLocationLine(clean)) {
    return false;
  }
  if (clean.length > 220) {
    return false;
...
function shouldStartImplicitBullet(line, previousBullet = '') {
  const clean = String(line || '').trim();
  const previous = String(previousBullet || '').trim();
  if (!clean) {
    return false;
  }
  if (!previous) {
  ...
  if (looksLikeWrappedBulletContinuation(clean)) {
    return false;
  }
  if (!/[.!?]$/.test(previous)) {
    return false;
  }
  return /^[A-Z0-9]/.test(clean) && clean.split(/\s+/).length >= 6;
}
...
/**
 * Reconstruct individual bullets from a dense paragraph that lacks bullet
 * markers. Used when DOCX/PDF extractors return achievements as one
 * concatenated sentence-block per role (common from mammoth's flat extraction
 * of paragraph-only resumes).
 *
 * Splits on sentence boundaries (`.`, `!`, `?`) followed by capitalized
 * sentence starters. Returns the original line if no clean split is found.
 *
 * @param {string} line
 * @returns {string[]} array of bullet strings (without leading "• ")
 */
function reconstructBulletsFromParagraph(line) {
  const clean = String(line || '').trim();
  if (!clean || clean.length < 100) {
    return [clean].filter(Boolean);
  }
  // Already has explicit bullet structure — leave it alone.
  if (BULLET_RE.test(clean)) {
    return [clean.replace(BULLET_RE, '').trim()];
  }
  // Don't split lines that are clearly headers, dates, or single phrases.
  if (DATE_RANGE_RE.test(clean) || isDateOnlyLine(clean)) {
    return [clean];
  }

  // Split on sentence-end punctuation followed by whitespace + capital letter,
  // a common pattern for sentence-cased achievement lists.
  const sentences = clean
    .split(/(?<=[.!?])\s+(?=[A-Z])/)
    .map(part => part.trim())
    .filter(Boolean);

  // Require at least 2 sentences AND each sentence to be a non-trivial length
  // before we accept the split — otherwise leave the line as a single bullet.
  if (sentences.length < 2) {
    return [clean];
  }
  if (sentences.some(sentence => sentence.split(/\s+/).length < 4)) {
    return [clean];
  }
  return sentences;
}
    ...
    if (BULLET_RE.test(line)) {
      ...
      if (isLikelyEntryHeader(line, nextLine)) {
        ...
        current = {
          ...
          dates:
            parsed.dates ||
            (nextLine && isDateOnlyLine(nextLine) ? normalizeDateLine(nextLine) : ''),
          bullets: [],
        };
        if (!parsed.dates && nextLine && isDateOnlyLine(nextLine)) {
          i += 1;
        }
      } else {
        const clean = line.trim();
        if (
          isLikelyNarrativeBulletLine(clean) &&
          shouldStartImplicitBullet(clean, current.bullets[current.bullets.length - 1])
        ) {
          current.bullets.push(clean);
        } else {
          current.bullets[current.bullets.length - 1] += ' ' + clean;
        }
      }
      ...
      if (current && current.bullets.length === 0 && !current.company && line.length < 80) {
        ...
        current.location = line.trim();
      } else if (current && isLikelyNarrativeBulletLine(line)) {
        // Reconstruct multiple bullets from a dense paragraph that lacks
        // explicit bullet markers (DOCX paragraph-only resumes).
        for (const bullet of reconstructBulletsFromParagraph(line.trim())) {
          current.bullets.push(bullet);
        }
      } else if (current) {
        if (current.bullets.length > 0) {
          const clean = line.trim();
          if (shouldStartImplicitBullet(clean, current.bullets[current.bullets.length - 1])) {
            current.bullets.push(clean);
          } else {
            current.bullets[current.bullets.length - 1] += ' ' + clean;
          }
...
function structureEducation(lines) {
  ...
  for (const line of lines) {
    ...
    const schoolWithDate = parseInstitutionDateLine(line);
    if (schoolWithDate) {
      const degree = groupLines.shift() || '';
      if (!degree) {
        const parsed = parseEducationGroup([schoolWithDate.school], schoolWithDate.dates);
      ...
      entries.push({
        degree: degree.trim(),
        school: schoolWithDate.school,
        dates: schoolWithDate.dates,
        details: groupLines.map(item => item.trim()).filter(Boolean),
      });
      groupLines = [];
      continue;
    }
...
function structureProjects(lines) {
  ...
  for (const line of lines) {
    if (BULLET_RE.test(line)) {
      ...
      const parsed = parseEntryHeader(clean);
      current = { name: parsed.title || clean, dates: parsed.dates || '', bullets: [] };
    }
  }
  if (current) {
    entries.push(current);
