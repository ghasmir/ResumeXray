...
function isLikelyEntryHeader(line) {
  ...
  if (DATE_RANGE_RE.test(clean)) return true;
  // Short, non-bullet line that looks like "Title - Company" or "Title, Company"
  if (!BULLET_RE.test(clean) && clean.length < 100 && /[-–—|]/.test(clean)) {
    return true;
  }
  if (!BULLET_RE.test(clean) && clean.length < 80 && /,\s*[A-Z]/.test(clean)) return true;
  return false;
}
...
function structureExperience(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    } else if (current && current.bullets.length > 0 && looksLikeWrappedBulletContinuation(line)) {
