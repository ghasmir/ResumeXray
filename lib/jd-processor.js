...
function extractJobTitleFromText(jdText = '') {
  ...
  for (const pattern of patterns) {
    ...
    if (match?.[1]) return sanitizeInput(match[1].trim().substring(0, 120));
  }
  ...
  for (const line of lines.slice(0, 10)) {
    if (/^(about|overview|company|summary|responsibilities|requirements|description)\b/i.test(line)) {
      continue;
    }
    if (/@|http|www\.|\bapply\b/i.test(line)) continue;
    if (line.split(/\s+/).length >= 2) return sanitizeInput(line.substring(0, 120));
  }
...
function extractCompanyFromText(jdText = '', jobTitle = '') {
  ...
  if (titleMatch?.[2]) return sanitizeInput(titleMatch[2].trim().substring(0, 120));
...
function normalizeJobContext(jobContext = {}) {
  ...
  return {
    ...
    jobTitle: sanitizeInput(jobContext.jobTitle || ''),
    companyName: sanitizeInput(jobContext.companyName || ''),
