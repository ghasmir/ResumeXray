...
// Sentence-fragment guards — page banners and aggregator instruction copy that
// often masquerade as titles when scraped from low-quality search pages
// (e.g. ie.indeed.com instructional headers, Glassdoor onboarding prompts).
const SENTENCE_FRAGMENT_TITLE_PATTERNS = [
  /^(?:please|provide|enter|select|fill|choose|click|see|view|read|browse|search|find|sign)\b/i,
  /^(?:we|our|your|their|this|that|these|those)\b/i,
  /^(?:about|overview|summary|responsibilities|requirements|benefits|salary|location|department|posted|employment\s+type)\b/i,
  /\.{2,}$/, // trailing ellipsis — truncated fragment
];
...
const INVALID_COMPANY_PATTERNS = [
  ...
  /^employer$/i,
  /^full job description$/i,
  // Plain aggregator brand names — these are page branding, never employer names.
  /^indeed$/i,
  /^linkedin$/i,
  /^glassdoor$/i,
  /^monster$/i,
...
function sanitizeJobTitleValue(value = '') {
  ...
  if (INVALID_JOB_TITLE_PATTERNS.some(pattern => pattern.test(clean))) {
    return '';
  }
  if (/^(responsibilities|requirements|benefits|salary|department|source|portal)$/i.test(clean)) {
    return '';
  }
  // Reject sentence fragments — long page-instruction copy and aggregator
  // boilerplate that slip through as the document <title>.
  if (clean.length > 80) {
    return '';
  }
  if (SENTENCE_FRAGMENT_TITLE_PATTERNS.some(pattern => pattern.test(clean))) {
    return '';
  }
  // Reject lines that end in a sentence-terminating punctuation mark — real
  // role titles never do (e.g. "Senior Engineer." is a sentence, not a title).
  // The trailing-ellipsis case is already handled by the patterns above.
...
function sanitizeCompanyNameValue(value = '') {
  ...
  if (INVALID_COMPANY_PATTERNS.some(pattern => pattern.test(clean))) {
    return '';
  }
  if (isAggregatorHostname(clean)) {
    return '';
  }
  return clean;
}
...
function hydrateAtsProfile(profileLike = {}) {
  ...
  return {
    ...
    defaultDensity:
      ...
      base.defaultDensity,
    singleColumn:
      profileLike.singleColumn ?? profileLike.templateProfile?.singleColumn ?? base.singleColumn,
    noTables: profileLike.noTables ?? profileLike.templateProfile?.noTables ?? base.noTables,
    strictHeaders:
      profileLike.strictHeaders ?? profileLike.templateProfile?.strictHeaders ?? base.strictHeaders,
    strictDates:
      profileLike.strictDates ?? profileLike.templateProfile?.strictDates ?? base.strictDates,
  };
}
...
function detectATS(jobUrl = '', jdText = '', scrapePlatform = '') {
  ...
  const normalizedPlatform =
    ATS_PLATFORM_ALIASES[scrapePlatform] || String(scrapePlatform || '').toLowerCase();
...
function extractCompanyFromUrl(jobUrl = '') {
  try {
    ...
    if (greenhouseMatch) {
      return toTitleCase(cleanSlug(greenhouseMatch));
    }
    ...
    if (workdayMatch?.[1]) {
      return toTitleCase(cleanSlug(workdayMatch[1]));
    }
    ...
    if (icimsMatch?.[1]) {
      ...
      return toTitleCase(cleanSlug(slug));
    }
...
function extractTitleFromUrl(jobUrl = '') {
  try {
    ...
    if (/greenhouse\.io$/.test(host) && segments.includes('jobs')) {
      ...
      if (segments[jobsIndex + 1]) {
        return toTitleCase(cleanSlug(segments[jobsIndex + 1]));
      }
    }
...
function extractCompanyFromText(jdText = '', jobTitle = '') {
  // Common sentence starts and section labels that should never be treated as
  // company names. Allow leading "The" so valid names like "The Trade Desk"
  // and "The Walt Disney Company" still resolve.
  const FALSE_POSITIVE_STARTS = new Set([
    'our',
    'we',
    'this',
    'that',
  ...
  const isFalsePositive = (str = '') => {
    ...
    if (FALSE_POSITIVE_STARTS.has(words[0])) {
      return true;
    }
    return words.every(w => w === 'the' || GENERIC_COMPANY_TERMS.has(w));
  };
  ...
  for (const pattern of patterns) {
    ...
    if (match?.[1]) {
      ...
      if (company && !isFalsePositive(company)) {
        return company;
      }
    }
  }
...
async function processJobDescription(jdInput, jobTitle = '', jobUrl = '') {
  ...
  const safeUrl = sanitizeInput(
    jobUrl || (safeInput.startsWith('http://') || safeInput.startsWith('https://') ? safeInput : '')
  );
  const pastedJd =
  ...
  const derivedTitle = sanitizeJobTitleValue(
    result.jobTitle || extractJobTitleFromText(result.jdText) || extractTitleFromUrl(safeUrl)
  );
  ...
  result.jobContext = normalizeJobContext({
    ...
    jdSource:
      result.jobContext.jdSource || (result.jdText ? 'pasted_text' : safeUrl ? 'url_only' : 'none'),
