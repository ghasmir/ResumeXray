...
function isLikelyJobTitleLine(line = '') {
  const clean = cleanExtractedValue(line, 90);
  if (!clean) return false;
  if (/@|https?:\/\/|www\.|^\d+$/.test(clean)) return false;
  if (
    /^(about|overview|company|summary|responsibilities|requirements|description|benefits|salary|location|department|source|portal|posted|employment type)\b/i.test(
      clean
    )
  ) {
    return false;
...
function extractCompanyFromText(jdText = '', jobTitle = '') {
  ...
  if (titleMatch?.[2]) return cleanExtractedValue(titleMatch[2]);
