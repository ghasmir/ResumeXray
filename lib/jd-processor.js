...
function extractCompanyFromText(jdText = '', jobTitle = '') {
  ...
  const FALSE_POSITIVE_STARTS = new Set([
    'our', 'we', 'this', 'that', 'your', 'their', 'there', 'these', 'those',
  ...
  const GENERIC_COMPANY_TERMS = new Set([
    'policy', 'purpose', 'role', 'position', 'job', 'overview', 'summary',
  ...
  const isFalsePositive = (str = '') => {
    ...
    if (FALSE_POSITIVE_STARTS.has(words[0])) return true;
  ...
  for (const pattern of patterns) {
    ...
    if (match?.[1]) {
      ...
      if (company && !isFalsePositive(company)) return company;
    }
  }
