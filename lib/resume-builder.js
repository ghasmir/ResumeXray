...
function isDecorativeHeaderLine(line) {
  ...
  if (/^(?:master|general|base|primary|final)\s+(?:cv|resume)$/i.test(clean)) {
    return true;
  }
...
const EXPERIENCE_ENTRY_SEPARATOR_RE = /\s+(?:[-–—]|\|)\s+/;
const EXPERIENCE_COMPANY_HINT_RE =
...
function looksLikeRoleTitleText(text) {
...
function looksLikeCompanyLabel(text) {
  const clean = String(text || '').trim();
  if (!clean || clean.length > 120 || /[.!?]$/.test(clean) || /^[a-z]/.test(clean)) {
    return false;
  }
  ...
  if (EXPERIENCE_COMPANY_HINT_RE.test(clean)) {
    return true;
  }
...
function looksLikeExperienceHeaderLine(line) {
  ...
  if (!clean || clean.length > 140 || /[.!?]$/.test(clean) || /^[a-z]/.test(clean)) {
    return false;
  }
  if (/^\d{2}\/\d{4}\s*[–-]\s*(?:\d{2}\/\d{4}|Present)$/i.test(clean)) {
    return true;
  }

  const withoutDates = clean
    .replace(/\b\d{2}\/\d{4}\s*[–-]\s*(?:\d{2}\/\d{4}|Present)\b/gi, '')
    .replace(/\b(19|20)\d{2}\s*[–-]\s*(?:Present|(19|20)\d{2})\b/gi, '')
    .trim();

  if (EXPERIENCE_ENTRY_SEPARATOR_RE.test(withoutDates)) {
    const match = withoutDates.match(/^(.+?)\s+(?:[-–—]|\|)\s+(.+)$/);
    if (match) {
      const left = match[1].trim();
      const right = match[2].trim();
      return (
        (looksLikeRoleTitleText(left) && looksLikeCompanyLabel(right)) ||
        (looksLikeRoleTitleText(right) && looksLikeCompanyLabel(left))
      );
    }
  }
  ...
  if (atMatch) {
    return looksLikeRoleTitleText(atMatch[1]) && looksLikeCompanyLabel(atMatch[2]);
  }
...
function buildResumeData(resumeText, sectionData, optimizedBullets, keywordPlan) {
  ...
  const originalExperienceCount = Array.isArray(sections.experience)
    ? sections.experience.filter(l => looksLikeExperienceHeaderLine(l)).length || 1
    : 1;
  highlightMetricsInSections(sections);
  trimForSinglePage(sections, { isJunior, maxPages });
  stripPlaceholderMetrics(sections);

  const postTrimExperienceCount = Array.isArray(sections.experience)
    ? sections.experience.filter(l => looksLikeExperienceHeaderLine(l)).length || 1
    : 1;
  if (postTrimExperienceCount < Math.ceil(originalExperienceCount * 0.75)) {
    const dropped = [];
    const keptSet = new Set(sections.experience);
    for (const line of Array.isArray(sections.experience._original)
      ? sections.experience._original
      : []) {
      if (!keptSet.has(line)) {
        dropped.push(line);
      }
    ...
    if (dropped.length > 0) {
      sections.experience = [...sections.experience, ...dropped];
    }
  }
...
function trimForSinglePage(sections, { isJunior, maxPages = 1 } = {}) {
  ...
  // Only aggressively trim bullet counts for <3 years experience.
  ...
  if (Array.isArray(sections.experience)) {
    ...
    let roleIndex = 0;
    let bulletsInRole = 0;
    let keepCurrentBulletContinuation = true;
    const trimmed = [];
    const BULLET_RE = /^[•\-*]\s/;
    ...
    for (const line of sections.experience) {
      ...
      const isStandaloneDateLine = /^\d{2}\/\d{4}\s*[–-]\s*(?:\d{2}\/\d{4}|Present)$/i.test(
        String(line || '').trim()
      );

      if (isHeader) {
        if (!isStandaloneDateLine) {
          roleIndex++;
          bulletsInRole = 0;
        }
        keepCurrentBulletContinuation = true;
        trimmed.push(line);
      } else if (isBullet) {
        if (roleIndex === 0) {
          roleIndex = 1;
        ...
        if (roleIndex > MAX_ROLES_WITH_BULLETS) {
          keepCurrentBulletContinuation = false;
          continue; // Skip bullets for very old roles
        }
        ...
        // If the parent bullet was trimmed out, drop its continuation lines too.
        if (keepCurrentBulletContinuation) {
          trimmed.push(line);
        }
      }
    }
...
async function renderHtmlToPdf(html, options = {}) {
  ...
  try {
    ...
    await page.evaluate(allowedPages => {
      ...
      if (allowedPages === 1 && !fitsOnSinglePage()) {
        trimEntryBullets('.section-experience .entry', index =>
          index === 0 ? 3 : index === 1 ? 2 : 1
        );
      }
      if (allowedPages === 1 && !fitsOnSinglePage()) {
