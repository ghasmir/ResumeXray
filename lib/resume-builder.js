...
function normalizeDocxDensityChoice(raw = '') {
  return String(raw || '').trim().toLowerCase() === 'compact' ? 'compact' : 'standard';
}
...
function resolveDocxTheme(template = 'refined', density = 'standard') {
  ...
  if (!compact) return { ...base, template: key, density: 'standard' };
...
function createDocxDocument(children, theme) {
  const filtered = children.reduce((acc, child, i) => {
    ...
    if (isEmpty && prevWasEmpty) return acc;
    if (isEmpty) { child._isEmptyParagraph = true; }
    else if (child._isEmptyParagraph) { delete child._isEmptyParagraph; }
    acc.push(child);
    return acc;
  }, []);
  filtered.forEach(c => { delete c._isEmptyParagraph; });
  return new Document({
    sections: [{
      properties: {
        page: {
...
function looksLikeLocationLine(line) {
  ...
  if (/[.!?]$/.test(clean)) return false;
  if (/\b(?:remote|hybrid|on-?site)\b/i.test(clean)) return true;
...
function isDecorativeHeaderLine(line) {
  const clean = String(line || '').trim();
  if (!clean) return false;
  if (/^(?:master|general|base|primary|final)\s+(?:cv|resume)$/i.test(clean)) return true;
  if (/\b(?:curriculum vitae|master cv|master resume)\b/i.test(clean)) return true;
  if (/\b(?:tech|engineering|data|technical|support|operations|product|design|sales)\s+roles?\b/i.test(clean)) {
    return true;
  }
...
function extractHeadlineFromHeaderBlock(text) {
  const lines = splitNonEmptyLines(text).filter(line => {
    ...
    if (isDecorativeHeaderLine(line)) return false;
...
function isLikelyNameLine(line) {
  ...
  if (isLikelyContactLine(clean) || looksLikeLocationLine(clean)) return false;
  ...
  return words.every(word => {
    ...
    return (
      ...
      !bannedTitleWords.test(stripped)
    );
  });
}
...
function looksLikeMeaningfulHeaderSummary(line) {
  const clean = String(line || '').trim();
  if (!clean) return false;
  if (isLikelyContactLine(clean) || isDecorativeHeaderLine(clean) || matchSectionHeader(clean)) return false;
  if (/\b\d+\+?\s+years? of experience\b/i.test(clean)) return true;
  if (/[.!?]$/.test(clean) && clean.split(/\s+/).length >= 8) return true;
...
function looksLikeCompanyLabel(text) {
  const clean = String(text || '').trim();
  if (!clean || clean.length > 120 || /[.!?]$/.test(clean) || /^[a-z]/.test(clean)) return false;

  const words = clean.split(/\s+/).filter(Boolean);
  if (words.length === 0 || words.length > 10) return false;
  if (EXPERIENCE_COMPANY_HINT_RE.test(clean)) return true;
...
function looksLikeExperienceHeaderLine(line) {
  ...
  if (!clean || clean.length > 140 || /[.!?]$/.test(clean) || /^[a-z]/.test(clean)) return false;
...
function buildResumeData(resumeText, sectionData, optimizedBullets, keywordPlan) {
  ...
  if (postTrimExperienceCount < Math.ceil(originalExperienceCount * 0.75)) {
    ...
    for (const line of (Array.isArray(sections.experience._original) ? sections.experience._original : [])) {
      if (!keptSet.has(line)) dropped.push(line);
    }
    if (dropped.length > 0) {
...
function hasResumeEvidence(term = '', evidenceText = '') {
  ...
  if (!clean || !String(evidenceText || '').trim()) return false;
...
function parseSectionsAdvanced(text) {
  ...
  const headerLines =
    firstSectionIndex === -1 ? lines.slice(0, Math.min(lines.length, 8)) : lines.slice(0, firstSectionIndex);
  const bodyStartIndex = firstSectionIndex === -1 ? headerLines.length : firstSectionIndex;
  const hasExplicitSummarySection = lines.slice(bodyStartIndex).some(line => matchSectionHeader(line) === 'summary');
  ...
  let currentSection = bodyStartIndex < lines.length ? matchSectionHeader(lines[bodyStartIndex]) || 'summary' : 'summary';
...
async function generateDOCX(resumeText, sectionData, optimizedBullets, keywordPlan, options = {}) {
  ...
  const addSection = (sectionKey, contentLines) => {
    ...
    docChildren.push(new Paragraph({
      ...
      border: { bottom: { style: BorderStyle.SINGLE, size: theme.sectionBorderSize, color: theme.dividerColor } }
    }));
  ...
  if (sections.strengths.length > 0) addSection('strengths', sections.strengths);
...
async function generatePDF(resumeText, sectionData, optimizedBullets, keywordPlan, options = {}) {
  ...
  if (isCoverLetter && coverLetterData) {
    ...
    const data = buildResumeData(resumeText, sectionData, optimizedBullets, keywordPlan);
    const templateName = ['refined', 'executive', 'corporate', 'modern', 'classic', 'minimal'].includes(template)
      ? template
      : 'refined';
