...
async function analyzeResume(resumeText, jdText = '') {
  ...
  try {
    aiShieldData = await llm.analyzeGhostingAndKnockouts(
      resumeText,
      jdText ||
        'No specific job description provided. Analyze the resume bullet points for general weakness: lack of metrics, passive language, vague duties without measurable outcomes.'
    );
  } catch (e) {
  ...
  let biasShield = { riskScore: 0, flags: [] };
  try {
    biasShield = await llm.analyzeBiasShield(resumeText);
  } catch (e) {
    log.warn('Bias Shield analysis failed', { error: e.message });
  }

  // 6. Generate recommendations based on the combined output
  const recommendations = generateRecommendations(keywordResults, xrayData, formatIssues);
