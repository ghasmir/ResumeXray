...
function runXrayAnalysis(rawText) {
  ...
  const expectedFields = ['Name', 'Email', 'Phone', 'Summary', 'Experience', 'Education', 'Skills'];
...
function simulateStructuredParser(text) {
  ...
  const phoneMatch = headerText.match(/(?:\+?\d{1,3}\s?)?(?:\(\d{1,4}\)|\d{1,4})(?:[-.\s]?\d{1,4}){2,}/);
  ...
  const sectionPatterns = {
    ...
    Skills: /(?:SKILLS|CORE COMPETENCIES|TECHNOLOGIES|STRENGTHS)/i,
