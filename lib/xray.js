...
function runXrayAnalysis(rawText) {
  ...
  expectedFields.forEach(field => {
    ...
    if (['Name', 'Email', 'Phone', 'Location', 'LinkedIn'].includes(field)) {
      extractedValue =
        structuredExtraction.Contact && structuredExtraction.Contact[field.toLowerCase()];
      if (extractedValue) {
    ...
    xrayData.fieldAccuracy[field] = {
      ...
      value:
        extractedValue ||
        '[Parser could not extract this field. Ensure it is not in a table or header.]',
    };
...
function simulateStructuredParser(text) {
  ...
  for (const line of headerLines) {
    ...
    if (line.match(/\d{5,}/)) {
      continue;
    } // phone/zip (5+ consecutive digits)
    if (line.match(/http|www\.|linkedin/i)) {
      continue;
    } // URLs
    if (line.length > 60) {
      continue;
  ...
  if (linkedinMatch) {
    sections.Contact.linkedin = linkedinMatch[0];
  }
  ...
  const locationMatch = headerText.match(
    /\b([A-Za-z]+(?:\s[A-Za-z]+)?),\s+([A-Z][a-z]{2,}(?:\s[A-Z][a-z]+)?|[A-Z]{2})\b/
  );
  if (locationMatch) {
    ...
    const boundary = Math.min(
      firstPipe > -1 ? firstPipe : Infinity,
      firstAt > -1 ? firstAt : Infinity
    );
  ...
  const phoneMatch = headerText.match(
    /(?:\+?\d{1,3}[\s.-]?)?(?:\(?\d{2,4}\)?[\s.-]?)?\d{2,4}[\s.-]\d{2,4}(?:[\s.-]\d{1,5})?/
  );
  if (phoneMatch) {
  ...
  const sectionPatterns = {
    ...
    Summary:
      /(?:SUMMARY|PROFILE|OBJECTIVE|ABOUT ME|ABOUT|CAREER OBJECTIVE|PROFESSIONAL SUMMARY|CAREER PROFILE)/i,
    ...
    if (line.length <= 50) {
      ...
      for (const [key, pattern] of Object.entries(sectionPatterns)) {
        if (pattern.test(line)) {
          ...
          const hasIcons = [...line].some(char => char.charCodeAt(0) > 127);
          if (hasIcons) {
            continue;
          } // Strict parser deterministically fails on non-ASCII headers
          foundSection = key;
          break;
        }
      }
