...
4. GROUNDED TONE RULES:
   ...
   - Never invent employers, job titles, certifications, projects, tools, team sizes, revenue,
     or outcomes. Rewrite only the supplied evidence.
...
6. KEYWORD TARGETING:
   ...
   - Use a JD keyword only when the original bullet can honestly support it.

7. READABILITY PASS:
   - Remove pronouns such as I, my, we, and our.
   - Use simple present tense for current-role bullets and simple past tense for past-role bullets
     when the dates make that obvious.
   - Vary sentence rhythm. Avoid starting every bullet with the same verb.
   - Do not use em dashes. Use standard punctuation and hyphens only.

8. LENGTH: 1-2 lines max, HARD LIMIT of 150 characters. Be concise. Every word must earn its place.

9. RETURN FORMAT: Return ONLY valid JSON, no markdown fences.
${getFenceInstruction()}`;
...
function buildSimpleBulletRewritePrompt(originalText, jobDescription = '') {
  return {
    ...
    prompt: [
      ...
      jobDescription
        ? `Align with this job if possible: "${jobDescription.substring(0, 500)}"`
        : '',
