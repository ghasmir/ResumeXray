...
const { sanitizeCompanyNameValue, sanitizeJobTitleValue } = require('../../jd-processor');
...
function buildCoverLetterPrompt(resumeText, jobDescription, jobContext = null) {
  ...
  if (safeRole) {
    contextLines.push(`Role: ${safeRole}`);
  }
  if (safeCompany) {
    contextLines.push(`Company: ${safeCompany}`);
  }
  if (jobContext?.jdSource) {
    contextLines.push(`Job Source: ${jobContext.jdSource}`);
  }
