...
/**
 * Extract numeric tokens from a bullet for fabrication-detection comparison.
 * Captures things like "12%", "$2.3M", "150+", "3x", "45.6", and bare integers.
 * Year-like 4-digit values (1900-2099) are excluded — they're rarely fabricated
 * and frequently appear in honest dates/calendars.
 *
 * @param {string} text
 * @returns {string[]} normalized lowercased numeric tokens
 */
function extractNumericTokens(text) {
  if (!text || typeof text !== 'string') {
    return [];
  }
  const matches = text.match(/\$?\d[\d,.]*[%+xkmb]?/gi) || [];
  return matches
    .map(token => token.toLowerCase().replace(/,/g, ''))
    .filter(token => {
      const bare = token.replace(/[^\d.]/g, '');
      // Drop calendar years (1900-2099) — these aren't "metrics" in the
      // fabrication sense.
      const numeric = parseFloat(bare);
      if (Number.isFinite(numeric) && numeric >= 1900 && numeric <= 2099 && /^\d{4}$/.test(bare)) {
        return false;
      }
...
/**
 * Detect numbers in `rewritten` that did not exist in `original`.
 * The LLM's bullet-rewrite contract requires placeholders (e.g. "[X]%") for
 * unsupported metrics; any concrete new number is treated as fabrication.
 *
 * @param {string} original
 * @param {string} rewritten
 * @returns {string[]} fabricated tokens found only in `rewritten`
 */
function detectFabricatedMetrics(original, rewritten) {
  const origTokens = new Set(extractNumericTokens(original));
  const newTokens = extractNumericTokens(rewritten);
  return newTokens.filter(token => !origTokens.has(token));
}
...
function validateBulletResult(result) {
  ...
  // Fabrication guard: if the rewrite contains numbers that weren't in the
  // original, the model has invented metrics. Revert to the original bullet
  // and flag needsMetric so downstream UI prompts the user for a real value.
  const fabricated = detectFabricatedMetrics(validated.original, validated.rewritten);
  if (fabricated.length > 0) {
    log.warn('LLM bullet rewrite introduced fabricated metrics — reverting to original', {
      fabricated,
      originalPreview: validated.original.substring(0, 80),
      rewrittenPreview: validated.rewritten.substring(0, 80),
    });
    validated.rewritten = validated.original;
    validated.needsMetric = true;
    validated.fabricatedMetrics = fabricated;
    validated.method = 'reverted: fabricated metric detected';
  }
