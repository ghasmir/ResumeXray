 ...
 *
 * To swap providers: change LLM_PROVIDER in .env (openai | gemini | puter-kimi)
...
class LlmQueue {
  ...
  add(fn) {
    // A-3 Fix: Reject when queue is full — prevents OOM under traffic spike.
    // Callers receive a 429-style error rather than silently queuing forever.
    if (this.queue.length >= 100) {
      return Promise.reject(new Error('LLM queue at capacity — please retry in a moment'));
    }
...
function getProvider() {
  ...
  switch (providerName) {
    ...
    case 'puter-kimi':
      return require('./providers/puter-kimi');
    default:
...
const {
  ...
  buildSimpleBulletRewritePrompt,
} = require('./prompts/bullet-rewrite');
...
async function computeEmbeddingScore(resumeText, jdText) {
  const [resumeVec, jdVec] = await Promise.all([getEmbedding(resumeText), getEmbedding(jdText)]);
  return calibrateScore(cosineSimilarity(resumeVec, jdVec));
}

// ── Utility: Parse JSON from LLM output ───────────────────────────────────────

// M-5 Fix: Robust JSON extractor that handles:
//   • Markdown triple-backtick fences: ```json ... ``` or ``` ... ```
//   • XML-style tags: <json>...</json> or <response>...</response>
//   • Leading/trailing single or double quotes
//   • Raw JSON (no wrapper)
function parseJsonFromLLM(text) {
  let cleaned = text.trim();

  // 1. Strip triple-backtick fences (most common LLM format)
  const fenceMatch = cleaned.match(/^```(?:json)?\s*([\s\S]*?)```\s*$/i);
  if (fenceMatch) {
    cleaned = fenceMatch[1].trim();
  } else {
    // 2. Strip XML-style wrappers occasionally emitted by Gemini/Claude
    const xmlMatch = cleaned.match(/^<(?:json|response)>([\s\S]*?)<\/(?:json|response)>$/i);
    if (xmlMatch) {
      cleaned = xmlMatch[1].trim();
    } else {
      // 3. Strip surrounding quotes (old behaviour, kept as last resort)
...
async function streamBulletRewrite(bulletText, jobDescription, onToken) {
  ...
  const fullText = await withRetry(() =>
    provider.stream(config.prompt, onToken, {
      systemPrompt: config.systemPrompt,
      model: config.model,
    })
  );
 ...
 */
async function streamCoverLetter(resumeText, jobDescription, onToken, jobContext = null) {
  const provider = getProvider();
  const config = buildCoverLetterPrompt(resumeText, jobDescription, jobContext);
 ...
 */
async function generateCoverLetter(resumeText, jobDescription, jobContext = null) {
  const provider = getProvider();
  const config = buildCoverLetterPrompt(resumeText, jobDescription, jobContext);
 ...
 */
async function streamKeywordInsertionPlan(
  resumeText,
  missingKeywords,
  onToken,
  jobDescription = ''
  ...
  const provider = getProvider();
  const config = buildKeywordPlanPrompt(resumeText, missingKeywords, jobDescription);
