 ...
 *
 * For production persistence, export keys with:
 *   node -e "const k=require('crypto').generateKeyPairSync('ed25519');
 *     console.log('PRIVATE:', k.privateKey.export({type:'pkcs8',format:'der'}).toString('base64'));
 *     console.log('PUBLIC:',  k.publicKey.export({type:'spki',format:'der'}).toString('base64'));"
 */
function getKeyPair() {
  if (keyPair) return keyPair;

  const privEnv = process.env.PROMPT_FENCE_PRIVATE_KEY;
  const pubEnv  = process.env.PROMPT_FENCE_PUBLIC_KEY;
  ...
  if (privEnv && pubEnv) {
    try {
      ...
      log.info('Prompt fence keys loaded from environment');
    } catch (err) {
...
function fenceUserContent(untrustedText, metadata = {}) {
  ...
  const signature = crypto.sign(null, Buffer.from(xml, 'utf-8'), privateKey)
    .toString('base64');
