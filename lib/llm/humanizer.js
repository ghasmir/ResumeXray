...
function auditContextRetention(original, rewritten) {
  ...
  if (droppedTech.length > 0) {
    ...
    result.warnings.push(
      `Technical context lost: "${droppedTech.join(', ')}" was in the original but removed from the rewrite.`
    );
  }
  ...
  const origMetrics =
    original.match(
      /\$[\d,.]+[MBKmk]?|\d+%|\d+x\b|\d+\+?\s*(?:users?|customers?|clients?|engineers?|developers?|team members?|people|employees?|servers?|requests?|endpoints?|transactions?|records?|queries?|deploys?|releases?)|\d+[MBK]\+?\s*(?:revenue|ARR|MRR|users?)|\d+\s*(?:months?|weeks?|days?|hours?|minutes?|years?)/gi
    ) || [];
  ...
  if (origMetrics.length > 0 && rewritMetrics.length === 0) {
    result.passed = false;
    result.warnings.push(
      `Metrics stripped: The original had quantifiable data ("${origMetrics[0]}") but the rewrite removed it.`
    );
  }
  ...
  const hasVagueResult =
    /\b(?:improved|increased|reduced|decreased|enhanced|boosted|grew|saved|accelerated|optimized|simplified|drove|delivered|managed|led|handled|built|cut|launched|completed|achieved|exceeded)\b/i.test(
      rewritten
    ) && !hasAnyMetric;

  if (hasVagueResult) {
    result.missingMetricPrompt =
      'This bullet claims an improvement but has no number. How much time, money, or effort did this save? (e.g., "reduced by 40%", "saved $12K/month", "cut from 3 days to 4 hours")';
  } else if (!hasAnyMetric && rewritten.length > 40) {
    result.missingMetricPrompt =
      'Adding a metric would make this bullet 3x more compelling. Can you estimate the impact? (e.g., team size, revenue affected, time saved)';
  }
  ...
  for (const noun of significantNouns) {
    if (!rewritten.includes(noun) && !rewritLower.includes(noun.toLowerCase())) {
      ...
      if (/^[A-Z]/.test(noun) && origLower.indexOf(noun.toLowerCase()) < origLower.length / 2) {
        result.warnings.push(
          `Context may be lost: "${noun}" was mentioned in original but not in rewrite. If this is a product/company name, it should be preserved.`
        );
      }
    }
  }
