...
const INJECTION_PATTERNS = [
  ...
  {
    pattern: /disregard\s+(all\s+)?(previous|prior|above|earlier)\s+/gi,
    label: 'instruction_override',
  },
  {
    pattern: /forget\s+(everything|all|what)\s+(you|i)\s+(said|told|know)/gi,
    label: 'instruction_override',
  },
  ...
  {
    pattern: /print\s+(your|the)\s+(system|initial|original)\s+(prompt|instructions)/gi,
    label: 'data_exfil',
  },
  {
    pattern: /what\s+(are|were)\s+your\s+(initial|system|original)\s+instructions/gi,
    label: 'data_exfil',
  },
];
...
function sanitizeForLLM(text) {
  ...
  cleaned = cleaned.replace(
    /<\s*\/?\s*(system|instruction|prompt|override|admin|root)\s*\/?>/gi,
    '[FILTERED:tag]'
  );
