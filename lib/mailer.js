...
const FROM_EMAIL =
  ...
  'noreply@resumexray-production-5320.up.railway.app';
...
async function sendVerificationEmail(to, token) {
  if (!sendFn) {
    log.warn('Verification email skipped (no transport configured)', {
      to: to.substring(0, 3) + '***',
    });
    return false;
  }
...
async function sendPasswordResetEmail(to, token) {
  if (!sendFn) {
    log.warn('Password reset email skipped (no transport configured)', {
      to: to.substring(0, 3) + '***',
    });
    return false;
  }
...
async function sendSSOLoginReminderEmail(to, provider) {
  if (!sendFn) {
    log.warn('SSO reminder email skipped (no transport configured)', {
      to: to.substring(0, 3) + '***',
    });
    return false;
  }
