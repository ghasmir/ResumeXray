...
async function sendSSOLoginReminderEmail(to, provider) {
  ...
  try {
    ...
    log.info('SSO login reminder email sent', { messageId, provider });
    return true;
  } catch (error) {
    log.error('Failed to send SSO login reminder email', { error: error.message });
    return false;
  }
}

module.exports = { sendVerificationEmail, sendPasswordResetEmail, sendSSOLoginReminderEmail };
