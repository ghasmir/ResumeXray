...
function checkFormatIssues(rawText, parsedSections) {
      ...
      issues: [
        {
          ...
          severity: 'high',
          message:
            'The parser found almost no text. This usually means your resume is a scanned image or flattened graphic.',
  ...
  if (multiColumnLines > 3) {
    issues.push({
      ...
      severity: 'high',
      message:
        'We detected large horizontal gaps, suggesting a multi-column layout. Parsers often merge these columns together, scrambling your experience.',
  ...
  const nonStandard = foundHeaders.filter(
    h => !standardHeaders.some(std => h.includes(std) || std.includes(h))
  );
  ...
  if (nonStandard.length > 0 && foundHeaders.length > 0) {
    ...
    issues.push({
      ...
      severity: 'high',
      message:
        'The parser could not identify standard sections (e.g., Work Experience, Education).',
  ...
  const dateRegex =
    /\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* (?:19|20)\d{2}\b|\b(?:0?[1-9]|1[0-2])\/(?:19|20)\d{2}\b/gi;
  const dates = rawText.match(dateRegex) || [];

  const hasAlphaDates = dates.some(d => /[a-z]/i.test(d));
  const hasNumericDates = dates.some(d => /\//.test(d));
  ...
  if (hasAlphaDates && hasNumericDates) {
    issues.push({
      ...
      severity: 'medium',
      message:
        'We found mixed date formats (e.g., "Jan 2022" and "01/2022"). This can confuse the parser\'s timeline calculations.',
  ...
  if (!parsedSections.Contact || Object.keys(parsedSections.Contact).length === 0) {
    issues.push({
      ...
      severity: 'high',
      message:
        'We could not extract an email address or phone number. If it is located in the document header or footer, the ATS might be ignoring it.',
  ...
  if (allCapsLines > lines.length * 0.1 && lines.length > 10) {
    // More than 10% of lines are ALL CAPS
    issues.push({
      id: 'excessive_caps',
      title: 'Excessive Capitalization',
      severity: 'low',
      message:
        'A significant portion of your resume is in ALL CAPS. While not a parser-killer, it can hurt readability and sometimes cause tokenization issues during semantic matching.',
  ...
  if (garbledRegex.test(rawText)) {
    issues.push({
      ...
      severity: 'high',
      message:
        "We detected unrecognized characters (). This usually means you used a non-standard font or the PDF wasn't flattened properly, causing the ATS to see gibberish.",
