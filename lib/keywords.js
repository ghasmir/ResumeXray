...
function extractKeywords(text) {
  ...
  // Contextual language detection for short ambiguous terms.
  if (/\b(?:golang|go(?:\s+language|\s+developer|\s+engineer|\s+microservices?|\s+backend))\b/i.test(text)) {
    freq.golang = (freq.golang || 0) + 1;
  }
  if (/\b(?:r(?:\s+programming|\s+language|\s+studio|\s+developer)?|rstudio|tidyverse)\b/i.test(text)) {
    freq.r = (freq.r || 0) + 1;
  }
  ...
  for (const [term, count] of Object.entries(freq)) {
    ...
    if (HARD_SKILLS.has(term)) {
      ...
      // Include recurring non-dictionary terms as "other" keywords
      otherKeywords.push(entry);
    }
  }

  // Also scan for dictionary terms that the tokenizer may have split
  // (e.g. "ci/cd" or "node.js" split by the tokenizer)
  for (const skill of HARD_SKILLS) {
    if (!freq[skill] && lower.includes(skill)) {
      hardSkills.push({ term: skill, count: 1 });
    }
  }

  if (freq.r) {
    hardSkills.push({ term: 'r', count: freq.r });
  }
