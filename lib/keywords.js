...
const REQUIREMENT_HEADING_PATTERNS = [
  ...
  {
    pattern:
      /^(essential requirements|requirements|required|qualifications|must have|must-haves)$/i,
    tier: 'essential',
  },
...
function extractKeywords(text) {
  ...
  if (
    /\b(?:golang|go(?:\s+language|\s+developer|\s+engineer|\s+microservices?|\s+backend))\b/i.test(
      raw
    )
  ) {
  ...
  if (
    /\b(?:r(?:\s+programming|\s+language|\s+studio|\s+developer)?|rstudio|tidyverse)\b/i.test(raw)
  ) {
    upsertEntry(hardSkills, {
      term: 'r',
      count: 1,
...
function categorize(term) {
  ...
  if (SOFT_SKILLS.has(term)) {
    return 'soft_skill';
  }
  if (CERTIFICATIONS.has(term)) {
...
function isHighSignalSoftSkill(entry = {}) {
  ...
  if (!LOW_SIGNAL_SOFT_SKILLS.has(entry.term)) {
    return true;
  }
  if (entry.explicitRequirement) {
    return (entry.importance || 0) >= 0.85;
  }
