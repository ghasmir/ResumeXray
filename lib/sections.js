...
const SECTION_PATTERNS = {
  contact: {
    ...
    patterns: [/\b(contact|personal\s+info|personal\s+details)\b/i],
  ...
  education: {
    label: 'Education',
    patterns: [/\b(education|academic|degree|university|college|school|qualification)\b/i],
    required: true,
  },
  skills: {
    label: 'Skills',
  ...
  certifications: {
    ...
    patterns: [/\b(certification|certifications|licenses|accreditation|credentials)\b/i],
    required: false,
  },
  projects: {
    label: 'Projects',
    patterns: [/\b(projects|portfolio|key\s+projects|selected\s+projects)\b/i],
    required: false,
  },
  awards: {
    label: 'Awards & Honors',
    patterns: [/\b(awards|honors|achievements|recognition|accomplishments)\b/i],
    required: false,
  },
  volunteer: {
    label: 'Volunteer Experience',
    patterns: [/\b(volunteer|community\s+service|pro\s+bono|civic)\b/i],
    required: false,
  },
  publications: {
    label: 'Publications',
    patterns: [/\b(publications|papers|research|journal|conference)\b/i],
    required: false,
  },
};
...
function analyzeFormat(resumeText) {
  ...
  if (verbs.length < 3) {
    warnings.push({
      ...
      message:
        'Use more action verbs (e.g., achieved, developed, implemented) to strengthen your bullet points.',
      severity: 'medium',
    });
  }
  ...
  if (quantified < 2) {
    warnings.push({
      type: 'few_metrics',
      message:
        'Add more quantified results (e.g., "increased revenue by 25%", "managed team of 12").',
      severity: 'medium',
    });
  }
...
function extractSections(text) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.length > 0 && trimmed.length < 30 && trimmed === trimmed.toUpperCase()) {
      if (trimmed.includes('EXPERIENCE')) {
        currentSection = 'Experience';
      } else if (trimmed.includes('EDUCATION')) {
        currentSection = 'Education';
      } else if (trimmed.includes('SKILLS')) {
        currentSection = 'Skills';
      } else if (trimmed.includes('SUMMARY') || trimmed.includes('PROFILE')) {
...
function extractContactInfo(text) {
  ...
  const headerLines = first10Lines
    .split('\n')
    .map(l => l.trim())
    .filter(l => l.length > 0);
  for (const line of headerLines) {
    if (line.match(/[@.]\S+\.\S+/) || line.match(/\d{3,}/) || line.match(/http|www\./i)) {
      continue;
    }
    if (line.length > 60) {
      continue;
    }
    if (line.match(/^(EXPERIENCE|EDUCATION|SKILLS|SUMMARY|PROFILE|OBJECTIVE|CONTACT|ADDRESS)/i)) {
      continue;
    }
    contact.name = line;
    break;
