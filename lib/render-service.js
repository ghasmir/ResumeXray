...
function buildRenderAttempts(jobContext) {
  ...
  const density =
    jobContext.templateProfile?.defaultDensity || atsProfile.defaultDensity || 'standard';
  ...
  for (const fallbackTemplate of qualityFallbacks) {
    if (template === fallbackTemplate && density === 'standard') {
      continue;
    }
    attempts.push({
...
function buildPreferredAttempt(jobContext, preferredTemplate, preferredDensity) {
  if (!preferredTemplate && !preferredDensity) {
    return null;
  }
  ...
  const template =
    preferredTemplate || jobContext.templateProfile?.template || baseProfile.template;
  const density =
...
async function renderResumePdf(scan, options = {}) {
  ...
  if (preferredAttempt) {
    renderAttempts.push(preferredAttempt);
  }

  for (const attempt of buildRenderAttempts(jobContext)) {
    if (
      renderAttempts.some(
        existing => existing.template === attempt.template && existing.density === attempt.density
      )
    ) {
      continue;
    }
