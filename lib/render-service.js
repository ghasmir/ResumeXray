const { buildResumeData, generatePDF, validatePDF } = require('./resume-builder');
const { hydrateAtsProfile, normalizeJobContext } = require('./jd-processor');

const ALLOWED_TEMPLATES = new Set(['refined', 'executive', 'corporate', 'modern', 'classic', 'minimal']);
const ALLOWED_DENSITIES = new Set(['standard', 'compact']);
...
function buildRenderAttempts(jobContext) {
  ...
  const template = jobContext.templateProfile?.template || atsProfile.template;
  const density = jobContext.templateProfile?.defaultDensity || atsProfile.defaultDensity || 'standard';
  ...
  const qualityFallbacks = ['refined', 'executive', 'corporate', 'classic'];
  for (const fallbackTemplate of qualityFallbacks) {
    if (template === fallbackTemplate && density === 'standard') continue;
    attempts.push({
      label: `quality-fallback-${fallbackTemplate}`,
  ...
  if (density !== 'compact') {
    attempts.push({
      label: 'resolved-profile-compact',
      template,
      density: 'compact',
      atsProfile: hydrateAtsProfile({
        ...atsProfile,
...
function normalizeTemplateChoice(value = '') {
  const choice = String(value || '').trim().toLowerCase();
  return ALLOWED_TEMPLATES.has(choice) ? choice : '';
}

function normalizeDensityChoice(value = '') {
  const choice = String(value || '').trim().toLowerCase();
  return ALLOWED_DENSITIES.has(choice) ? choice : '';
}

function buildPreferredAttempt(jobContext, preferredTemplate, preferredDensity) {
  if (!preferredTemplate && !preferredDensity) return null;

  const baseProfile = hydrateAtsProfile({
    name: jobContext.atsPlatform,
    displayName: jobContext.atsDisplayName,
    templateProfile: jobContext.templateProfile,
  });

  const template = preferredTemplate || jobContext.templateProfile?.template || baseProfile.template;
  const density =
    preferredDensity || jobContext.templateProfile?.defaultDensity || baseProfile.defaultDensity;
...
async function renderResumePdf(scan, options = {}) {
  ...
  const previewData = buildResumeData(resumeText, sectionData, optimizedBullets, keywordPlan);
  const maxPages = previewData.maxPages || 1;

  const attempts = [];
  const preferredTemplate = normalizeTemplateChoice(options.template);
  const preferredDensity = normalizeDensityChoice(options.density);
  const renderAttempts = [];
  const preferredAttempt = buildPreferredAttempt(jobContext, preferredTemplate, preferredDensity);
  if (preferredAttempt) renderAttempts.push(preferredAttempt);

  for (const attempt of buildRenderAttempts(jobContext)) {
    if (
      renderAttempts.some(
        existing =>
          existing.template === attempt.template && existing.density === attempt.density
      )
    ) {
      continue;
    }
    renderAttempts.push(attempt);
  }
  ...
  for (const attempt of renderAttempts) {
    try {
      ...
      const isPageOverflowOnly =
        !validation.valid &&
        typeof validation.error === 'string' &&
        validation.error.startsWith('PDF exceeds ');

      if (isPageOverflowOnly && (validation.textLength || 0) >= 90) {
        return {
          buffer,
          renderMeta: {
            ...renderMeta,
