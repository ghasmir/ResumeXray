...
async function runAgentPipeline(resumeText, jdText, emitter, options = {}) {
  ...
  const atsProfile = options.atsProfile || {
    name: 'generic',
  ...
  const fencedResumeText = fenceUserContent(cleanResumeText, {
    source: 'user_upload',
    type: 'resume',
  });
  const fencedJdText = jdText
    ? fenceUserContent(cleanJdText, { source: 'user_input', type: 'job_description' })
    : '';
  ...
  if (ef.Email || ef.Phone) {
    ...
    if (linkedinMatch) {
      parts.push(linkedinMatch[0]);
    }

    // Try to extract location — look for "City, Country" or "City, State" patterns
    const locationMatch = headerBlock.match(
      /\b([A-Z][a-z]+(?:\s[A-Z][a-z]+)?),?\s+(Ireland|UK|USA|US|India|Canada|Australia|Germany|France|Netherlands|Remote)\b/i
    );
    if (locationMatch) {
      parts.push(locationMatch[0].trim());
    }
  ...
  if (!integrity.isResume) {
    ...
    emitter.emitError(
      'This document does not appear to be a professional resume. Optimization skipped.',
      1
    );
  ...
  emitter.emitStep(1, 'parse', 'complete', 'Resume parsed', {
    ...
    sections: Object.values(sectionData.sections)
      .filter(s => s.found)
      .map(s => s.label),
  });
  ...
  emitter.emitStep(
    ...
    'format',
    'complete',
    `Format health: ${Math.round(formatResults.formatHealth)}%`
  );
  ...
  const aiTasks = [
    ...
    cleanJdText
      ? llm.analyzeSemanticMatch(cleanResumeText, cleanJdText)
      : Promise.resolve({ score: 0, missing: [], inferred: [] }),
  ...
  emitter.emitStep(
    ...
    'keywords',
    'complete',
    `Keywords: ${keywordResults?.matched?.length || 0} matched`
  );
  ...
  emitter.emitStep(
    ...
    'ghosting',
    'complete',
    `Audit: ${aiShieldData.ghostingBullets.length} weak points found`
  );
  ...
  const allMissing = [
    ...new Set(
      (keywordResults?.missing || [])
        .filter(item => item?.category === 'hard_skill' || item?.explicitRequirement)
        .map(item => item.term)
    ),
  ].slice(0, options.limitKeywords || 15);

  if (allMissing.length > 0) {
    emitter.emitStep(8, 'keywords-plan', 'running', 'Generating keyword insertion plan...');
    try {
      const keywordPlan = await llm.streamKeywordInsertionPlan(
        cleanResumeText,
        allMissing,
        chunk => {
      ...
      if (Array.isArray(keywordPlan)) {
        for (const plan of keywordPlan) {
          plan._instruction =
            'Rewrite and reorder only; do not remove or skip any experience entries.';
        }
      }
  ...
  if (cleanJdText && cleanJdText.trim().length > 50) {
    ...
    try {
      const coverLetter = await llm.streamCoverLetter(
        coverLetterInput,
        cleanJdText,
        token => {
  ...
  const atsAfterVal = validateScore(
    Math.min(100, atsReady + 15 + optimizedBullets.length * 2),
    'ats'
  );
  emitter.emitScores({
    parseRate: safeParseRate,
