...
async function runAgentPipeline(resumeText, jdText, emitter, options = {}) {
  ...
  const atsProfile = options.atsProfile || { name: 'generic', displayName: 'ATS-Optimized', template: 'refined', singleColumn: false, noTables: true, strictHeaders: true };
    ...
    try {
      ...
      if (Array.isArray(keywordPlan)) {
        for (const plan of keywordPlan) {
          plan._instruction = 'Rewrite and reorder only; do not remove or skip any experience entries.';
        }
      }
