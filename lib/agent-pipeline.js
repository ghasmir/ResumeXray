...
async function runAgentPipeline(resumeText, jdText, emitter, options = {}) {
  const maxRewrites = options.maxBulletRewrites || 5;
  ...
  const lockedCount = Math.max(0, (aiShieldData.ghostingBullets?.length || 0) - maxRewrites);
  ...
  if (bulletsToRewrite.length > 0) {
    ...
    emitter.emitStep(7, 'rewrite', 'complete', `Optimized ${optimizedBullets.length} bullets`);
    if (lockedCount > 0) {
      emitter.emitStep(7, 'rewrite', 'locked', `${lockedCount} more improvements available — purchase credits to unlock`);
    }
  } else {
  ...
  for (const opt of optimizedBullets) {
    if (opt.original && opt.rewritten) {
      // Use split/join to replace ALL occurrences (String.replace with a string only replaces the first)
      optimizedText = optimizedText.split(opt.original).join(opt.rewritten);
    }
  }
  results.optimizedResumeText = optimizedText;

    if (cleanJdText && cleanJdText.trim().length > 50) {
    emitter.emitStep(9, 'cover-letter', 'running', 'Generating tailored cover letter...');
    try {
      const coverLetter = await llm.streamCoverLetter(optimizedText, cleanJdText, (token) => {
