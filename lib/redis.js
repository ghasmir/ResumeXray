...
function degradeRedis(reason, err = null) {
  if (!fallbackLogged) {
    log.error(reason, err ? { error: err.message } : {});
    fallbackLogged = true;
  }
...
function getRedis() {
  ...
  try {
    redis = new Redis(url, {
      ...
      retryStrategy(times) {
        if (times > 2) {
          degradeRedis('Redis max retries reached — falling back to local');
          return null; // Stop retrying
        }
        return Math.min(times * 200, 3000);
      },
      tls: url.startsWith('rediss://') ? {} : undefined, // empty = rejectUnauthorized: true (default)
    });
    ...
    redis.on('error', (err) => {
      ...
      if (/timed out|ETIMEDOUT|ECONNREFUSED|ENOTFOUND|Connection is closed/i.test(err.message)) {
        degradeRedis('Redis became unavailable — falling back to local', err);
      }
    });
    ...
    degradeRedis('Redis initialization failed — falling back to local', err);
    return null;
  }
}
